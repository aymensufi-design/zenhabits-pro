...
def sync_to_sheets(date_str, task, status, mood_score):
    ...
    if sheet:
        try:
            sheet.append_row([str(date_str), task, int(status), float(mood_score)])
        except:
            pass
...
<style>
    ...
    .track-text { font-family: 'Dancing Script', cursive; color: #AD1457; font-size: 55px; text-align: center; }
    ...
    .speed-text { font-weight: bold; font-size: 20px; text-align: center; }
    .stButton>button { background: #AD1457 !important; color: white !important; border-radius: 25px; }
</style>
...
def auth_page():
    if bin_str:
        st.markdown(f'<p align="center"><img src="data:image/jpeg;base64,{bin_str}" width="150"></p>', unsafe_allow_html=True)
...
def dashboard():
    ...
    with st.sidebar:
        if bin_str:
            st.markdown(f'<p align="center"><img src="data:image/jpeg;base64,{bin_str}" width="100"></p>', unsafe_allow_html=True)
    ...
    with col_l:
        ...
        if day_tasks:
                ...
                if search in t_name.lower():
                    ...
                    with ca:
                        ...
                        if check and not t_stat:
                            ...
                            conn.commit()
                            sync_to_sheets(target_date, t_name, 1, 0) 
                            time.sleep(8) 
                            reward_area.empty()
                            st.rerun()
                    with cb: 
                        st.markdown(f'<div class="habit-card">{i}. {t_name} (⏰ {t_time})</div>', unsafe_allow_html=True)
                    with cc:
            ...
            if st.button("💾 Save Progress & Mood"):
                ...
                conn.commit()
                sync_to_sheets(target_date, "Journal Update", 1, score)
    ...
    with col_r:
        ...
        if not df_mood.empty:
            ...
            fig_mood.update_layout(height=200, paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', showlegend=False, font_color="white")
            st.plotly_chart(fig_mood, use_container_width=True)
        else:
            st.info("Journal entries will show here!")
